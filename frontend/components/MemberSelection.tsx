      ...
      <div className="md:col-span-2">
        ...
        <div className="flex flex-wrap gap-2">
          {visibleMembers.map(member => (  // Use visibleMembers here to only show selected members
            <span
              key={member}
              className="inline-flex items-center gap-1 bg-amber-100 text-amber-800 px-3 py-1.5 rounded-full text-sm font-medium"
            >
          ...
          {visibleMembers.length === 0 && (
            <span className="text-sm text-stone-400">No members selected</span>
          )}
        </div>
      </div>
    </div>
